/**
  ...
  Updates by P. F. Roysdon for NSCNC Elara with 4-th axis:
  - 12/22/2022: Changed max tool table length from 256 to 999 to accomodate more tools
  - 12/22/2022: Added spindle stop (M5) and coolant off (M9) commands prior to program end (M30)
  - 12/31/2022: Added two options (with and without menu) for 4-th axis milling
*/
...
properties = {
  ...
  /**
  fourthAxis: { // PFR added 12/31/2022
...
// PFR added 12/31/2022
  if (true) {
    var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], range:[-360,360], cyclic:true, preference:0});
...
/** This works. PFR added 12/31/2022
  if (getProperty("fourthAxis") != "none") {
    var aAxis = createAxis({coordinate:0, table:true, axis:[(getProperty("fourthAxis") == "x") ? 1 : 0, (getProperty("fourthAxis") == "y") ? 1 : 0, 0], range:[-360,360], cyclic:true, preference:0});
    machineConfiguration = new MachineConfiguration(aAxis);
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    if (tool.number > 999) { //(tool.number > 256) { // PFR changed 12/22/2022
  ...
  if (insertToolCall || retracted) {
    ...
    if (lengthOffset > 999) {  //(lengthOffset > 256) { //PFR changed 12/22/2022
...
function onClose() {
  ...
  writeBlock(mFormat.format(5)); // spindle stop, PFR added 12/22/2022
  
  writeBlock(mFormat.format(9)); // coolant off, PFR added 12/22/2022
  
  //setCoolant(COOLANT_OFF);  //PFR removed 12/22/2022
  ...
  //onImpliedCommand(COMMAND_STOP_SPINDLE);  //PFR removed 12/22/2022
